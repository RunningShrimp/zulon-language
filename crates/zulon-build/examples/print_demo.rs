...
//! Print Example
//!
//! This example demonstrates using the print function
//! to output values from a ZULON program.
...
fn main() {
    ...
    let print_function = create_print_function();
    ...
    match pipeline.build() {
        Ok(exe_path) => {
            ...
            println!();
            println!("💡 Run it with: ./print_example");
...
fn create_print_function() -> LirFunction {
    ...
    // Create basic block that calls print and returns 0
    let block = LirBlock {
        id: 0,
        ...
        instructions: vec![
            ...
            // Note: In a real compiler, we would emit a call instruction
            // to zulon_print_i32 here. For now, we just return the value.
        ],
